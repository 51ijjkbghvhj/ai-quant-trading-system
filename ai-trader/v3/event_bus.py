...
MARKET_JSON = BASE_DIR.parent / "trading-dashboard" / "market_data.json"
TRADE_LOG = BASE_DIR / "ai_trade.log"
...
def print_system_banner():
    ...
    print("=" * 60)
    print("  [System] AI Trading System v9.0.1 - Status Report")
    print("=" * 60)
...
FALLBACK_STOCKS = {
    ...
    "sh601398": "工商银行", "sz002594": "比亚迪", "sz000001": "平安银行",
    "sz300059": "东方财富", "sz002230": "科大讯飞", "sz300308": "中际旭创",
    "sh688981": "中芯国际", "sz300263": "隆华科技"
}
STOCK_POOL = set(FALLBACK_STOCKS.keys())
...
def update_market_json(current_data, pf, events, indices, news, monitor_pool_codes=None):
    """更新市场数据供前端展示"""
    import os
    MARKET_JSON = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "trading-dashboard", "market_data.json")
...
RESEARCH_CONFIG_PATH = os.path.join(os.path.dirname(__file__), '..', 'ai_config_research.json')
...
def execute_trade(signal: dict, portfolio: dict, market_data: dict, market_stage: str = "neutral") -> bool:
    ...
    try:
        ...
        # ★ 修复:从portfolio实时获取cash,避免使用过期变量
        cash = portfolio.get("cash", 10000)
...
def run_cycle(research_mode=False):
    ...
    try:
        ...
        # ★ 关键修复:同步更新 pf 中持仓的现价
        # 否则后续计算总资产 (total) 会偏低,导致风控误判单只仓位超限
        ...
        # 更新收益曲线
        try:
            pnl_hist_path = WORKSPACE / "ai-trader" / "pnl_history.json"
            pnl_hist = {"history": []}
            ...
            init = pf.get("initial_cash", 10000)
