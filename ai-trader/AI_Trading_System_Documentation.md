...
### 11.0.3 v9.0.6 (2026-05-25 15:40)
**动态防守线（Dynamic Defense Line）+ 前端双列展示（止损/止盈分离）**

#### 🛡️ 核心新增：动态防守线机制
| 模块 | 详情描述 |
|:---|:---|
| **🔢 数学计算 (`v3/event_bus.py`)** | 在 AI 分析前计算每只持仓的 `risk_line`：<br>• **初始保护**：盈利 < 3% 时，`风险线 = 成本价 × (1 - 初始回撤)`（主板 5%，创业板 7%）<br>• **移动止盈**：盈利 ≥ 3% 时，`风险线 = 历史最高价 × (1 - 动态回撤)`（主板 3%，创业板 6%）<br>• **单向锚定**：股价上涨则线上移，股价下跌则线钉死不动 |
| **🚨 风险拦截器 (Hard Guardrail)** | 在 AI 给出建议后、实际下单前进行强制校验：<br>• 若 `现价 < risk_line` 且非 T+1 锁定状态，系统强制触发 SELL 信号<br>• 卖出时使用 `risk_line * 0.98` 限价单，确保即时成交（防逃单） |

#### 🎨 前端 UI 重构 (`server.py` + `index.html`)
| 组件 | 修改内容 |
|:---|:---|
| **后端数据注入 (`server.py`)** | `pos_display` 增加 `profit_pct`（盈亏比）和 `risk_line`（防守线）计算逻辑，确保前端能拿到状态判断依据 |
| **双列拆分展示** | 废弃原本单一的“动态防守线”列，拆分为 **【🛡️ 止损保护】**（橙色）与 **【🔒 止盈锁定】**（绿色）<br>• 盈利 < 3%：数字显示在止损列，强调“保命”<br>• 盈利 ≥ 3%：数字显示在止盈列，强调“锁利” |
| **安全垫可视化** | 根据 `(现价 - 风险线) / 现价` 计算距离：<br>• `> 4%`：🟢 安全<br>• `1.5% ~ 4%`：🟡 警戒<br>• `< 1.5%` 或跌破：🔴 触发 |

#### 🔧 缺陷修复
| 修复项 | 详情描述 |
|:---|:---|
| **💥 UnboundLocalError (`server.py`)** | 修复新增计算逻辑时遗漏的 `total_qty` 变量定义报错 |
| **💥 NameError (`server.py`)** | 修复直接引用未定义 `cur_price` 导致的崩溃，改为从批次安全获取 |
