...
PF = USER_DATA / "portfolio.json"
LOG = USER_DATA / "ai_trade.log"
STATIC = BASE / "trading-dashboard" / "static"
MODE_FILE = USER_DATA / "system_mode.json"
AI_CONFIG = USER_DATA / "ai_config.json"
PORT = 8503

# 自动迁移旧数据
_old_pf = BASE / "ai-trader" / "portfolio.json"
if not PF.exists() and _old_pf.exists():
    print(f"[Server] 🔄 正在迁移旧版 portfolio.json 到 user_data...")
    import shutil
    shutil.copy2(_old_pf, PF)

_old_ai = BASE / "ai-trader" / "ai_config.json"
if not AI_CONFIG.exists() and _old_ai.exists():
    print(f"[Server] 🔄 正在迁移旧版 ai_config.json 到 user_data...")
    import shutil
    shutil.copy2(_old_ai, AI_CONFIG)
...
def print_system_banner():
    ...
    try:
        ...
        start_layer(str(PF)) # 🆕 使用新的 USER_DATA 路径
    except Exception as e:
        print(f"[基本面层] 启动失败: {e}")
...
def get_stats():
    try:
        db_path = USER_DATA / "trading_history.db"
...
def api():
    ...
    try:
        ...
        # 获取研究 AI 模型名称 (🆕 指向 user_data)
        try:
            research_cfg = USER_DATA / "ai_config_research.json"
            with open(research_cfg, 'r', encoding='utf-8') as f:
        ...
        for sym in list(pos.keys()):
            ...
            for b in batches:
                ...
                current = b.get("current_price", 0)
                if current > 0 and risk_line > 0:
                    b["distance"] = round((current - risk_line) / current * 100, 2)
                else:
                    b["distance"] = 0
        ...
        try:
            for sym, entry in pos.items():
                ...
                # 🟢 计算盈亏比例，用于前端判断显示“止损”还是“止盈”
                total_qty = s_qty + l_qty
                cur_price = batches[0].get('current_price', 0)
                p['cost_price'] = round((s_avg * s_qty + l_avg * l_qty) / total_qty, 3) if total_qty > 0 else 0
                p['current_price'] = cur_price # 确保前端拿到最新价
                current_profit_pct = (cur_price - p['cost_price']) / p['cost_price'] if p['cost_price'] > 0 else 0
                p['profit_pct'] = round(current_profit_pct, 4)

                # 🟢 提取风控线 (取所有批次中最严格的线)
                min_rl = 999999
                has_rl = False
                for b in batches:
                    rl = b.get('risk_line', 0)
                    if rl > 0:
                        has_rl = True
                        if rl < min_rl: min_rl = rl
                p['risk_line'] = min_rl if has_rl else 0
...
class H(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if p in ("/","/index.html"):
            ...
            rc_path = USER_DATA / "ai_config_research.json"
            if rc_path.exists():
